...
describe('RulesPage', () => {
  it('publishes the main scoring awards and how they accumulate', () => {
    ...
    expect(
      screen.getByText(/os três critérios principais acumulam até 15 pontos/i),
    ).toBeInTheDocument();
  });
  ...
  it('explains penalty shootout scoring and the maximum score', () => {
    ...
    expect(
      screen.getByText(
        /nos jogos com pênaltis, o vencedor escolhido nos pênaltis conta como o vencedor/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /placar exato com vencedor nos pênaltis incorreto ou ausente vale somente os 5 pontos do placar exato/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/categorias secundárias somam no máximo 13 pontos/i),
    ).toBeInTheDocument();
  ...
  it('retains the secondary scoring categories', () => {
    ...
    expect(
      screen.getByText(/em 0–0 não há pontos por primeiro gol ou artilheiro/i),
    ).toBeInTheDocument();
  });
  ...
  it('publishes the prizes for the top three places', () => {
    ...
    expect(screen.getByText('2º lugar — 3 caipirinhas')).toBeInTheDocument();
    expect(screen.getByText('3º lugar — 2 capirinhas')).toBeInTheDocument();
  });
});
